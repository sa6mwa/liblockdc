...
test_query_index_prepared_exact_cache_is_namespace_and_generation_scoped(
  ...
  (void)state;
  test_root_path(root, sizeof(root), "query-index-prepared-exact-cache");
  test_cleanup_root(root);
  test_allocator_init(&allocator, &tracked);
  memset(&error, 0, sizeof(error));
  memset(&opts, 0, sizeof(opts));
  ...
  opts.content_type = "application/json";
  source = source_from_text("{\"kind\":\"hot\"}");
...
static void test_query_index_prepared_exact_cache_ignores_owner_and_key_filters(
    void **state) {
  char root[256];
  lc_pouch_allocator allocator;
  tracked_allocator tracked;
  ...
  (void)state;
  test_root_path(root, sizeof(root), "query-index-prepared-exact-filter-cache");
  test_cleanup_root(root);
  test_allocator_init(&allocator, &tracked);
  memset(&error, 0, sizeof(error));
  memset(&opts, 0, sizeof(opts));
  ...
  opts.content_type = "application/json";
  source = source_from_text("{\"kind\":\"hot\",\"group\":\"fast\"}");
  rc = store->write_state(store, "default", "alpha", source, &opts, &alpha,
                          &error);
  lc_source_close(source);
  assert_int_equal(rc, LC_OK);
...
test_query_index_prepared_exists_cache_is_namespace_and_generation_scoped(
  ...
  (void)state;
  test_root_path(root, sizeof(root), "query-index-prepared-exists-cache");
  test_cleanup_root(root);
  test_allocator_init(&allocator, &tracked);
  memset(&error, 0, sizeof(error));
  memset(&opts, 0, sizeof(opts));
  ...
  opts.content_type = "application/json";
  source = source_from_text("{\"tag\":\"yes\"}");
  rc = store->write_state(store, "default", "alpha", source, &opts, &alpha,
                          &error);
  lc_source_close(source);
  assert_int_equal(rc, LC_OK);
...
test_query_index_prepared_prefix_cache_is_namespace_and_generation_scoped(
  ...
  (void)state;
  test_root_path(root, sizeof(root), "query-index-prepared-prefix-cache");
  test_cleanup_root(root);
  test_allocator_init(&allocator, &tracked);
  memset(&error, 0, sizeof(error));
  memset(&opts, 0, sizeof(opts));
  ...
  opts.content_type = "application/json";
  source = source_from_text("{\"name\":\"alpha\"}");
  rc = store->write_state(store, "default", "alpha", source, &opts, &alpha,
                          &error);
  lc_source_close(source);
  assert_int_equal(rc, LC_OK);
  ...
  source = source_from_text("{\"name\":\"alpine\"}");
  rc = store->write_state(store, "other", "bravo", source, &opts, &bravo,
                          &error);
  lc_source_close(source);
  assert_int_equal(rc, LC_OK);
  ...
  source = source_from_text("{\"name\":\"alphabet\"}");
  rc = store->write_state(store, "default", "charlie", source, &opts, &charlie,
                          &error);
  lc_source_close(source);
  assert_int_equal(rc, LC_OK);
...
test_query_index_prepared_contains_cache_is_namespace_and_generation_scoped(
  ...
  (void)state;
  test_root_path(root, sizeof(root), "query-index-prepared-contains-cache");
  test_cleanup_root(root);
  test_allocator_init(&allocator, &tracked);
  memset(&error, 0, sizeof(error));
  memset(&opts, 0, sizeof(opts));
  ...
  opts.content_type = "application/json";
  source = source_from_text("{\"body\":\"xxneedle-one\"}");
  rc = store->write_state(store, "default", "alpha", source, &opts, &alpha,
                          &error);
  lc_source_close(source);
  assert_int_equal(rc, LC_OK);
  ...
  source = source_from_text("{\"body\":\"other-needle-two\"}");
  rc = store->write_state(store, "other", "bravo", source, &opts, &bravo,
                          &error);
  lc_source_close(source);
  assert_int_equal(rc, LC_OK);
  ...
  source = source_from_text("{\"body\":\"third needle value\"}");
  rc = store->write_state(store, "default", "charlie", source, &opts, &charlie,
                          &error);
  lc_source_close(source);
  assert_int_equal(rc, LC_OK);
...
static void test_query_index_temporal_generation_is_lazy_after_writes(
  ...
  (void)state;
  test_root_path(root, sizeof(root), "query-index-temporal-generation-lazy");
  test_cleanup_root(root);
  test_allocator_init(&allocator, &tracked);
  memset(&error, 0, sizeof(error));
  memset(&opts, 0, sizeof(opts));
  ...
  test_query_temporal_generation_path(root, "default", generation_path,
                                      sizeof(generation_path));
  assert_int_not_equal(access(generation_path, F_OK), 0);
  ...
  assert_int_equal(keys.count, 1U);
  assert_string_equal(keys.keys[0], "alpha");
  assert_int_equal(access(generation_path, F_OK), 0);
  lc_pouch_query_index_scan_res_cleanup(&allocator, &scan);
...
static void test_query_index_in_deduplicates_duplicate_values(void **state) {
  ...
  write_query_range_number_state(&allocator, store, "zeta",
                                 "{\"tags\":[\"finance\"]}", &error);
  ...
  assert_string_equal(keys.keys[1], "finance");
  assert_string_equal(keys.keys[2], "planning");
  assert_string_equal(keys.keys[3], "zeta");
  assert_false(scan.truncated);
  assert_true(scan.index_seq > 0UL);
  lc_pouch_query_index_scan_res_cleanup(&allocator, &scan);
  ...
  assert_string_equal(rows.keys[1], "finance");
  assert_string_equal(rows.keys[2], "planning");
  assert_string_equal(rows.keys[3], "zeta");
  assert_false(scan.truncated);
  lc_pouch_query_index_scan_res_cleanup(&allocator, &scan);
...
static void
test_query_index_eq_not_eq_result_cache_subtracts_negative_terms(void **state) {
  char root[256];
  lc_pouch_allocator allocator;
  tracked_allocator tracked;
  lc_pouch_store *store;
  ...
  (void)state;
  test_root_path(root, sizeof(root), "query-index-eq-not-eq-cache");
  test_cleanup_root(root);
  test_allocator_init(&allocator, &tracked);
  memset(&error, 0, sizeof(error));
  memset(&eq, 0, sizeof(eq));
...
static void
test_query_index_in_eq_result_cache_reuses_compound_plan(void **state) {
  char root[256];
  lc_pouch_allocator allocator;
  tracked_allocator tracked;
  lc_pouch_store *store;
  ...
  (void)state;
  test_root_path(root, sizeof(root), "query-index-in-eq-cache");
  test_cleanup_root(root);
  test_allocator_init(&allocator, &tracked);
  memset(&error, 0, sizeof(error));
  memset(&in, 0, sizeof(in));
  ...
  write_query_range_number_state(&allocator, store, "alpha",
                                 "{\"tag\":\"planning\",\"kind\":\"include\"}",
                                 &error);
  write_query_range_number_state(&allocator, store, "bravo",
                                 "{\"tag\":\"finance\",\"kind\":\"exclude\"}",
                                 &error);
  write_query_range_number_state(&allocator, store, "charlie",
                                 "{\"tag\":\"ops\",\"kind\":\"include\"}",
                                 &error);
...
static void
test_query_index_in_not_eq_result_cache_subtracts_negative_terms(void **state) {
  char root[256];
  lc_pouch_allocator allocator;
  tracked_allocator tracked;
  lc_pouch_store *store;
  ...
  (void)state;
  test_root_path(root, sizeof(root), "query-index-in-not-eq-cache");
  test_cleanup_root(root);
  test_allocator_init(&allocator, &tracked);
  memset(&error, 0, sizeof(error));
  memset(&in, 0, sizeof(in));
  ...
  write_query_range_number_state(&allocator, store, "alpha",
                                 "{\"tag\":\"planning\",\"status\":\"allow\"}",
                                 &error);
  write_query_range_number_state(&allocator, store, "bravo",
                                 "{\"tag\":\"finance\",\"status\":\"block\"}",
                                 &error);
  write_query_range_number_state(&allocator, store, "charlie",
                                 "{\"tag\":\"ops\",\"status\":\"allow\"}",
                                 &error);
...
test_query_index_exists_eq_result_cache_reuses_compound_plan(void **state) {
  ...
  (void)state;
  test_root_path(root, sizeof(root), "query-index-exists-eq-cache");
  test_cleanup_root(root);
  test_allocator_init(&allocator, &tracked);
  memset(&error, 0, sizeof(error));
  memset(&exists, 0, sizeof(exists));
  ...
  write_query_range_number_state(&allocator, store, "alpha",
                                 "{\"tag\":\"yes\",\"kind\":\"include\"}",
                                 &error);
  write_query_range_number_state(&allocator, store, "bravo",
                                 "{\"tag\":\"yes\",\"kind\":\"exclude\"}",
                                 &error);
  write_query_range_number_state(&allocator, store, "charlie",
                                 "{\"kind\":\"include\"}", &error);
...
int main(void) {
  const struct CMUnitTest tests[] = {
      ...
      cmocka_unit_test(
          test_query_index_prepared_exact_cache_is_namespace_and_generation_scoped),
      cmocka_unit_test(
          test_query_index_prepared_exact_cache_ignores_owner_and_key_filters),
      cmocka_unit_test(
          test_query_index_prepared_exists_cache_is_namespace_and_generation_scoped),
      cmocka_unit_test(
          test_query_index_prepared_range_cache_is_namespace_and_generation_scoped),
      ...
      cmocka_unit_test(
          test_query_index_eq_not_eq_result_cache_subtracts_negative_terms),
      cmocka_unit_test(
          test_query_index_in_eq_result_cache_reuses_compound_plan),
      cmocka_unit_test(
