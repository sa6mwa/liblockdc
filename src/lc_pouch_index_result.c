...
static int lc_pouch_index_result_plan_has_cacheable_eq_filters(
    const lc_pouch_query_index_scan_req *req) {
  size_t index;
  ...
  if (req->document_eq_terms == NULL) {
    return 0;
  }
  for (index = 0U; index < req->document_eq_term_count; ++index) {
    if (req->document_eq_terms[index].field == NULL ||
        req->document_eq_terms[index].value == NULL) {
      return 0;
    }
  }
  return 1;
...
static int lc_pouch_index_result_plan_date_after_has_only_implied_exists(
    const lc_pouch_query_index_scan_req *req) {
  const char *field;

  if (req == NULL || req->document_date_after_terms == NULL ||
  ...
  if (req->document_exists_term_count == 0U) {
    return 1;
  }
  if (req->document_exists_term_count != 1U ||
      req->document_exists_terms == NULL ||
      req->document_exists_terms[0].field == NULL) {
    return 0;
  }
  field = req->document_date_after_terms[0].field;
  return strcmp(req->document_exists_terms[0].field, field) == 0;
}
...
static int lc_pouch_index_result_plan_date_after_cacheable(
  ...
  return lc_pouch_index_result_plan_cacheable_common(req) &&
         ...
         lc_pouch_index_result_plan_has_no_prefix_filters(req) &&
         lc_pouch_index_result_plan_has_no_contains_filters(req) &&
         lc_pouch_index_result_plan_date_after_has_only_implied_exists(req);
}
...
lc_pouch_index_result_sorted_eq_terms(const lc_pouch_allocator *allocator,
  ...
  count = req->document_eq_term_count;
  ...
  for (index = 0U; index < count; ++index) {
    terms[index].field = req->document_eq_terms[index].field;
    terms[index].value = req->document_eq_terms[index].value;
  }
...
lc_pouch_index_range_result_plan_key(const lc_pouch_allocator *allocator,
  ...
  eq_terms = NULL;
  not_eq_terms = NULL;
  eq_count = req->document_eq_term_count;
  eq_unique_count = 0U;
  if (eq_count > 0U) {
    if (eq_count >
        ((size_t)-1) / sizeof(lc_pouch_index_eq_result_plan_key_term)) {
      return NULL;
    }
    eq_terms = (lc_pouch_index_eq_result_plan_key_term *)lc_pouch_alloc(
        allocator, eq_count * sizeof(eq_terms[0]));
    if (eq_terms == NULL) {
      return NULL;
    }
    for (eq_index = 0U; eq_index < eq_count; ++eq_index) {
      eq_terms[eq_index].field = req->document_eq_terms[eq_index].field;
      eq_terms[eq_index].value = req->document_eq_terms[eq_index].value;
    }
    qsort(eq_terms, eq_count, sizeof(eq_terms[0]),
          lc_pouch_index_eq_result_plan_key_term_compare);
    for (eq_index = 0U; eq_index < eq_count; ++eq_index) {
      if (eq_unique_count > 0U &&
          strcmp(eq_terms[eq_index].field,
                 eq_terms[eq_unique_count - 1U].field) == 0 &&
          strcmp(eq_terms[eq_index].value,
                 eq_terms[eq_unique_count - 1U].value) == 0) {
        continue;
      }
      eq_terms[eq_unique_count++] = eq_terms[eq_index];
    }
  }
  not_eq_terms = lc_pouch_index_result_sorted_not_eq_terms(
      allocator, req, &not_eq_unique_count);
  ...
  cursor = key;
  offset = 0U;
  written = snprintf(cursor, needed - offset,
                     "range:%lu:%s:%lu:%s:%d:%lu:%s:%d:%lu:%s:%d:%lu:%s:%d:%"
                     "lu:%s",
                     (unsigned long)namespace_len, req->namespace_name,
                     (unsigned long)field_len, term->field,
...
lc_pouch_index_text_result_plan_key(const lc_pouch_allocator *allocator,
  ...
  cursor = key;
  offset = 0U;
  written = snprintf(cursor, needed - offset, "%s:%lu:%s:%lu:%s:%d:%lu:%s",
                     kind, (unsigned long)namespace_len, req->namespace_name,
                     (unsigned long)field_len, field, ignore_case ? 1 : 0,
                     (unsigned long)value_len, value);
  offset += (size_t)written;
  cursor = key + offset;
