...
static int lc_pouch_disk_query_exists_result_cacheable(
    const lc_pouch_query_index_scan_req *req) {
  if (req == NULL || req->namespace_name == NULL || req->key != NULL ||
      req->owner != NULL || req->document_exists_terms == NULL ||
...
static char *lc_pouch_disk_query_exists_result_cache_key(
    lc_pouch_disk_store *store, const lc_pouch_query_index_scan_req *req) {
  const lc_pouch_document_exists_term *term;
  size_t namespace_len;
  size_t field_len;
  ...
  if (!lc_pouch_disk_query_exists_result_cacheable(req)) {
    return NULL;
  }
...
static int lc_pouch_disk_query_in_value_ptr_compare(const void *left,
                                                    const void *right) {
...
static int lc_pouch_disk_query_in_result_cacheable(
    const lc_pouch_query_index_scan_req *req) {
  if (req == NULL || req->namespace_name == NULL || req->key != NULL ||
      req->owner != NULL || req->document_in_terms == NULL ||
...
static char *lc_pouch_disk_query_in_result_cache_key(
    lc_pouch_disk_store *store, const lc_pouch_query_index_scan_req *req) {
  ...
  if (!lc_pouch_disk_query_in_result_cacheable(req)) {
    return NULL;
  }
  ...
  values = (const char **)lc_pouch_alloc(&store->allocator,
  ...
  qsort(values, value_count, sizeof(values[0]),
        lc_pouch_disk_query_in_value_ptr_compare);
  unique_count = 0U;
  ...
  if (written < 0) {
    lc_pouch_free(&store->allocator, values);
    return NULL;
  }
  ...
  key = (char *)lc_pouch_alloc(&store->allocator, total_len + 1U);
  if (key == NULL) {
    lc_pouch_free(&store->allocator, values);
    return NULL;
  }
...
static int lc_pouch_disk_query_prefix_result_cacheable(
    const lc_pouch_query_index_scan_req *req) {
  if (req == NULL || req->namespace_name == NULL || req->key != NULL ||
      req->owner != NULL || req->document_prefix_terms == NULL ||
...
static int lc_pouch_disk_query_contains_result_cacheable(
    const lc_pouch_query_index_scan_req *req) {
  if (req == NULL || req->namespace_name == NULL || req->key != NULL ||
      req->owner != NULL || req->document_contains_terms == NULL ||
...
static char *lc_pouch_disk_query_text_result_cache_key(
    lc_pouch_disk_store *store, const lc_pouch_query_index_scan_req *req,
    const char *kind, const char *field, const char *value, int ignore_case) {
  size_t namespace_len;
  size_t field_len;
  size_t value_len;
  int written;
  ...
  if (store == NULL || req == NULL || kind == NULL || field == NULL ||
      value == NULL) {
    return NULL;
  }
  namespace_len = strlen(req->namespace_name);
...
static char *lc_pouch_disk_query_prefix_result_cache_key(
    lc_pouch_disk_store *store, const lc_pouch_query_index_scan_req *req) {
  const lc_pouch_document_prefix_term *term;

  if (!lc_pouch_disk_query_prefix_result_cacheable(req)) {
    return NULL;
  }
  term = &req->document_prefix_terms[0];
  return lc_pouch_disk_query_text_result_cache_key(
      store, req, "prefix", term->field, term->value, term->ignore_case);
}

static char *lc_pouch_disk_query_contains_result_cache_key(
    lc_pouch_disk_store *store, const lc_pouch_query_index_scan_req *req) {
  const lc_pouch_document_contains_term *term;

  if (!lc_pouch_disk_query_contains_result_cacheable(req)) {
    return NULL;
  }
  term = &req->document_contains_terms[0];
  return lc_pouch_disk_query_text_result_cache_key(
      store, req, "contains", term->field, term->value, term->ignore_case);
}
...
static int lc_pouch_disk_query_field_collect_exists_summary_indices_locked(
  ...
  *index_count_out = 0U;
  ...
  indices = NULL;
  index_count = 0U;
  cache_key = lc_pouch_disk_query_exists_result_cache_key(store, req);
  cache_generation = lc_pouch_disk_index_sequence(store);
  cache_hit =
      cache_key != NULL && lc_pouch_index_result_cache_find(
                               &store->allocator, &store->query_result_cache,
  ...
  if (!cache_hit) {
    ...
    reader.in_from = 0U;
    reader.require_summary_match = 1;
    reader.use_prepared_exists_cache = cache_key != NULL;
    reader.alloc_message = "failed to allocate pouch exists query docIDs";
    rc = lc_pouch_index_collect_exists_term_doc_ids(
        &store->allocator, primary,
        lc_pouch_disk_query_read_exists_term_doc_ids, &reader, &doc_ids, error);
    if (rc != LC_OK) {
      lc_pouch_free(&store->allocator, cache_key);
      lc_pouch_disk_exact_term_doc_id_reader_cleanup(&reader);
      lc_pouch_index_doc_id_set_cleanup(&store->allocator, &doc_ids);
...
static int lc_pouch_disk_query_field_exists_scan_locked(
  ...
  if (out->truncated && visit_count > 0U) {
    ...
    if (out->next_start_after == NULL) {
      ...
      lc_pouch_disk_unlock(store, error);
      return lc_pouch_set_nomem(error,
                                "failed to allocate pouch exists query cursor");
    }
  }
  out->index_seq = lc_pouch_disk_index_sequence(store);
  lc_pouch_free(&store->allocator, indices);
...
static int lc_pouch_disk_query_field_collect_in_summary_indices_locked(
  ...
  *index_count_out = 0U;
  ...
  }
  {
    ...
    memset(&doc_ids, 0, sizeof(doc_ids));
    memset(&reader, 0, sizeof(reader));
    cache_key = lc_pouch_disk_query_in_result_cache_key(store, req);
    cache_generation = lc_pouch_disk_index_sequence(store);
...
static int lc_pouch_disk_query_field_collect_prefix_keys_locked(
  ...
  *key_count_out = 0U;
  ...
  indices = NULL;
  index_count = 0U;
  cache_key = lc_pouch_disk_query_prefix_result_cache_key(store, req);
  cache_generation = lc_pouch_disk_index_sequence(store);
  cache_hit =
      cache_key != NULL && lc_pouch_index_result_cache_find(
                               &store->allocator, &store->query_result_cache,
  ...
  if (!cache_hit) {
    ...
    reader.require_summary_match = 1;
    reader.require_positive_terms_summary_match = 1;
    reader.use_prepared_prefix_cache = cache_key != NULL;
    reader.alloc_message = "failed to allocate pouch prefix query docIDs";
    rc = lc_pouch_index_collect_prefix_term_doc_ids(
        &store->allocator, primary,
        lc_pouch_disk_query_read_prefix_term_doc_ids, &reader, &doc_ids, error);
    if (rc != LC_OK) {
      lc_pouch_free(&store->allocator, cache_key);
      lc_pouch_disk_exact_term_doc_id_reader_cleanup(&reader);
      lc_pouch_index_doc_id_set_cleanup(&store->allocator, &doc_ids);
...
static int lc_pouch_disk_query_field_prefix_scan_locked(
  ...
  keys = NULL;
  key_count = 0U;
  rc = lc_pouch_disk_query_field_collect_prefix_keys_locked(store, req, &keys,
                                                            &key_count, error);
  if (rc != LC_OK) {
    lc_pouch_disk_unlock(store, error);
    return rc;
  ...
  for (index = start_index; index < key_count && row_index < visit_count;
    ...
    if (!lc_pouch_disk_copy_summary_for_scan(store, &rows[row_index], entry)) {
      ...
      lc_pouch_disk_query_key_array_cleanup(store, keys, key_count);
      lc_pouch_disk_unlock(store, error);
      return lc_pouch_set_nomem(error, "failed to copy pouch prefix query row");
    }
    row_index++;
  }
  if (out->truncated && visit_count > 0U) {
    ...
    if (out->next_start_after == NULL) {
      ...
      lc_pouch_disk_unlock(store, error);
      return lc_pouch_set_nomem(error,
                                "failed to allocate pouch prefix query cursor");
    }
  }
  out->index_seq = lc_pouch_disk_index_sequence(store);
  lc_pouch_disk_query_key_array_cleanup(store, keys, key_count);
...
static int lc_pouch_disk_query_field_collect_contains_summary_indices_locked(
  ...
  *index_count_out = 0U;
  ...
  indices = NULL;
  index_count = 0U;
  cache_key = lc_pouch_disk_query_contains_result_cache_key(store, req);
  cache_generation = lc_pouch_disk_index_sequence(store);
  cache_hit =
      cache_key != NULL && lc_pouch_index_result_cache_find(
                               &store->allocator, &store->query_result_cache,
  ...
  if (!cache_hit) {
    ...
    reader.require_summary_match = 1;
    reader.require_positive_terms_summary_match = 1;
    reader.use_prepared_contains_cache = cache_key != NULL;
    reader.alloc_message = "failed to allocate pouch contains query docIDs";
    rc = lc_pouch_index_collect_contains_term_doc_ids(
        &store->allocator, primary,
        lc_pouch_disk_query_read_contains_term_doc_ids, &reader, &doc_ids,
        error);
    if (rc != LC_OK) {
      lc_pouch_free(&store->allocator, cache_key);
      lc_pouch_disk_exact_term_doc_id_reader_cleanup(&reader);
...
static int lc_pouch_disk_query_field_contains_scan_locked(
  ...
  if (out->truncated && visit_count > 0U) {
    ...
    if (out->next_start_after == NULL) {
      ...
      lc_pouch_disk_unlock(store, error);
      return lc_pouch_set_nomem(
          error, "failed to allocate pouch contains query cursor");
    }
  }
  out->index_seq = lc_pouch_disk_index_sequence(store);
  lc_pouch_free(&store->allocator, indices);
...
static int lc_pouch_disk_query_field_contains_keys_scan_locked(
  ...
  indices = NULL;
  rc = lc_pouch_disk_query_field_collect_contains_summary_indices_locked(
      store, req, &indices, &key_count, error);
  if (rc != LC_OK) {
    lc_pouch_disk_unlock(store, error);
    return rc;
  }
...
static int lc_pouch_disk_query_field_scan_locked(
  ...
  if (out->truncated && visit_count > 0U) {
    ...
    if (out->next_start_after == NULL) {
      ...
      lc_pouch_disk_unlock(store, error);
      return lc_pouch_set_nomem(error,
                                "failed to allocate pouch field query cursor");
    }
  }
  out->index_seq = lc_pouch_disk_index_sequence(store);
  lc_pouch_free(&store->allocator, indices);
