...
typedef struct lc_pouch_queue_stream_source_ctx {
  char *header;
  size_t header_length;
  size_t header_offset;
  lc_source *payload;
  size_t payload_bytes;
} lc_pouch_queue_stream_source_ctx;
...
static char *lc_pouch_namespace_config_key(const char *namespace_name,
  ...
  key = (char *)lc_alloc_with_allocator(NULL, prefix_length +
                                                  namespace_length * 2U + 1U);
  if (key == NULL) {
...
static int
lc_pouch_query_scan_key_collect(const lc_pouch_state_visit_entry *entry,
                                void *context, lc_error *error) {
  ...
  key.has_query_hidden = entry->has_query_hidden;
  key.query_hidden = entry->query_hidden;
  return lc_pouch_query_index_key_set_add(
      (lc_pouch_query_index_key_set *)context, &key, error);
}
...
static int
lc_pouch_query_index_visit_exact_key(const lc_pouch_query_index_key_view *key,
                                     void *context, lc_error *error) {
  lc_pouch_query_scan_context *scan;
  int rc;
  ...
  if (rc == LC_OK && scan != NULL && scan->page_full) {
...
static int lc_pouch_query_run_scan_predicate(lc_pouch_query_scan_context *scan,
  ...
  read_keys = NULL;
  rc = lc_pouch_state_visit(scan->client->pouch, scan->namespace_name,
  ...
  if (rc == LC_OK && keys.count > 0U) {
    ...
    if (read_keys == NULL) {
      rc = lc_error_set(error, LC_ERR_NOMEM, 0L,
                        "failed to allocate pouch scanned query read keys",
                        NULL, NULL, NULL);
    }
  }
  if (rc == LC_OK && keys.count > 0U) {
  ...
  *flushed_seq_out = 0UL;
  ...
  if (rc == LC_OK && plan.range) {
    ...
    rc = lc_pouch_query_index_visit_range(
        scan->client->pouch, scan->namespace_name, plan.field,
        &plan.range_bounds, lc_pouch_query_index_key_collect, &keys, &value_seq,
        error);
    if (rc == LC_OK && value_seq > scan->index_seq) {
      scan->index_seq = value_seq;
    }
...
static int lc_pouch_timestamp_add(long base, long delta, const char *field,
                                  long *out, lc_error *error) {
  char message[128];
...
static int lc_pouch_attach_write_locked(void *context, lc_error *error) {
  ...
  if (lc_pouch_txn_id_present(ctx->req->lease.txn_id)) {
    return lc_pouch_state_stage_write(
        ctx->client->pouch, ".lockd/attachments", ctx->attachment_key,
        ctx->req->lease.txn_id, ctx->source, ctx->options, ctx->result, error);
  }
  return lc_pouch_state_write(ctx->client->pouch, ".lockd/attachments",
...
static int lc_pouch_txn_collect_key(const lc_pouch_state_visit_entry *entry,
  ...
  prefix = "txn/";
  if (entry->key == NULL || strncmp(entry->key, prefix, strlen(prefix)) != 0) {
    return LC_OK;
  }
...
static char *lc_pouch_queue_state_key(const char *queue, const char *message_id,
                                      lc_error *error) {
  char *key;
  size_t queue_length;
...
static int lc_pouch_queue_record_source(const lc_pouch_queue_record *record,
  ...
  if (record->payload == NULL && record->payload_length > 0U) {
    return lc_error_set(error, LC_ERR_INVALID, 0L,
                        "pouch queue record payload is not materialized", NULL,
                        NULL, NULL);
  }
...
static size_t lc_pouch_queue_stream_source_read(void *context, void *buffer,
                                                size_t count, lc_error *error) {
  lc_pouch_queue_stream_source_ctx *source;
  size_t available;
  size_t copied;
  ...
  source = (lc_pouch_queue_stream_source_ctx *)context;
  if (source->header_offset < source->header_length) {
    available = source->header_length - source->header_offset;
    copied = count < available ? count : available;
    memcpy(buffer, source->header + source->header_offset, copied);
    source->header_offset += copied;
    return copied;
  }
  copied = source->payload->read(source->payload, buffer, count, error);
  source->payload_bytes += copied;
  return copied;
}

static int lc_pouch_queue_stream_source_reset(void *context, lc_error *error) {
  lc_pouch_queue_stream_source_ctx *source;
  int rc;

  if (context == NULL) {
    return lc_error_set(error, LC_ERR_INVALID, 0L,
                        "pouch queue stream reset requires context", NULL, NULL,
                        NULL);
  }
  source = (lc_pouch_queue_stream_source_ctx *)context;
  if (source->payload == NULL || source->payload->reset == NULL) {
    return lc_error_set(error, LC_ERR_INVALID, 0L,
                        "pouch queue payload source is not resettable", NULL,
                        NULL, NULL);
  }
  rc = source->payload->reset(source->payload, error);
  if (rc == LC_OK) {
    source->header_offset = 0U;
    source->payload_bytes = 0U;
  }
  return rc;
}

static void lc_pouch_queue_stream_source_close(void *context) {
  lc_pouch_queue_stream_source_ctx *source;
  ...
  source = (lc_pouch_queue_stream_source_ctx *)context;
  lc_free_with_allocator(NULL, source->header);
  lc_free_with_allocator(NULL, source);
}

static int lc_pouch_queue_stream_source_open(
    const lc_pouch_queue_record *record, lc_source *payload,
    int include_payload_length, lc_pouch_queue_stream_source_ctx **context_out,
    lc_source **out, lc_error *error) {
  lc_pouch_queue_stream_source_ctx *context;
  int rc;

  if (record == NULL || payload == NULL || out == NULL) {
    return lc_error_set(error, LC_ERR_INVALID, 0L,
                        "pouch queue stream source requires record, payload, "
                        "and out",
                        NULL, NULL, NULL);
  }
  if (context_out != NULL) {
  ...
  *out = NULL;
  context = (lc_pouch_queue_stream_source_ctx *)lc_calloc_with_allocator(
      NULL, 1U, sizeof(*context));
  if (context == NULL) {
    return lc_error_set(error, LC_ERR_NOMEM, 0L,
                        "failed to allocate pouch queue stream source", NULL,
                        NULL, NULL);
  }
  context->payload = payload;
  rc = lc_pouch_queue_record_header(record, include_payload_length,
                                    &context->header,
                                    &context->header_length, error);
  if (rc == LC_OK) {
    rc = lc_source_from_callbacks(
        lc_pouch_queue_stream_source_read, lc_pouch_queue_stream_source_reset,
        lc_pouch_queue_stream_source_close, context, out, error);
  }
  if (rc != LC_OK) {
    lc_pouch_queue_stream_source_close(context);
    return rc;
  }
  if (context_out != NULL) {
    *context_out = context;
...
static int
lc_pouch_queue_stream_source(const lc_pouch_queue_record *record,
                             lc_source *payload,
                             lc_pouch_queue_stream_source_ctx **context_out,
                             lc_source **out, lc_error *error) {
  return lc_pouch_queue_stream_source_open(record, payload, 0, context_out, out,
                                          error);
}

static int
lc_pouch_queue_record_stream_source(const lc_pouch_queue_record *record,
                                    lc_source *payload, lc_source **out,
                                    lc_error *error) {
  return lc_pouch_queue_stream_source_open(record, payload, 1, NULL, out,
                                          error);
}
...
static char *lc_pouch_lease_storage_key(const char *namespace_name,
                                        const char *key, lc_error *error) {
  char *namespace_hex;
  char *key_hex;
  char *storage_key;
  ...
  if (namespace_hex == NULL || key_hex == NULL) {
    ...
    lc_free_with_allocator(NULL, key_hex);
    (void)lc_error_set(error, LC_ERR_NOMEM, 0L,
                       "failed to allocate pouch lease key", NULL, NULL, NULL);
    return NULL;
  }
...
static void lc_pouch_generate_lease_id(char *buffer, size_t buffer_size) {
  ...
  now_seconds = (long)time(NULL);
  snprintf(buffer, buffer_size, "pouch-lease-%ld-%lu", now_seconds, sequence);
}

static int
lc_pouch_lease_record_parse(lc_client_handle *client,
...
static int lc_pouch_read_lease_record(lc_client_handle *client,
  ...
  rc = lc_pouch_state_read(client->pouch, LC_POUCH_LEASE_NAMESPACE, storage_key,
                           &read_result, error);
  if (rc == LC_OK) {
    rc = lc_pouch_lease_record_parse(client, &read_result, record, error);
  }
...
static int lc_pouch_write_lease_record_locked(void *context, lc_error *error) {
  ...
  return lc_pouch_state_write(ctx->client->pouch, LC_POUCH_LEASE_NAMESPACE,
                              ctx->storage_key, ctx->source, ctx->options,
                              ctx->out, error);
}
...
static int lc_pouch_queue_scan_load(lc_client_handle *client,
  ...
  rc = lc_pouch_state_visit(client->pouch, ".lockd/queue", lc_pouch_queue_visit,
                            scan, error);
...
static int lc_pouch_queue_payload_source(lc_client_handle *client,
  ...
  if (client == NULL || storage_key == NULL || out == NULL) {
    return lc_error_set(error, LC_ERR_INVALID, 0L,
                        "pouch queue payload source requires client, key, and "
                        "out",
                        NULL, NULL, NULL);
  }
  *out = NULL;
...
static int lc_pouch_queue_open_record_source(lc_client_handle *client,
  ...
  *out = NULL;
  ...
  rc = lc_pouch_queue_payload_source(client, record->storage_key,
                                     payload_source_out, error);
  if (rc == LC_OK) {
    rc = lc_pouch_queue_record_stream_source(record, *payload_source_out, out,
                                             error);
  }
  if (rc != LC_OK && *payload_source_out != NULL) {
    lc_source_close(*payload_source_out);
    *payload_source_out = NULL;
  }
  return rc;
}
...
static int lc_pouch_queue_write_record(lc_client_handle *client,
  ...
  if (rc == LC_OK) {
    ...
    rc =
        lc_pouch_state_write(client->pouch, ".lockd/queue", record->storage_key,
...
static int lc_pouch_queue_make_message(lc_client_handle *client,
  ...
  if (record->payload != NULL || record->payload_length == 0U) {
    ...
    rc = lc_pouch_queue_payload_source(client, record->storage_key, &payload,
                                       error);
  }
  if (rc != LC_OK) {
    return rc;
...
static char *lc_pouch_txn_key(const char *txn_id, lc_error *error) {
  ...
  key = (char *)lc_alloc_with_allocator(NULL,
                                        strlen(txn_id) + strlen("txn/") + 1U);
  if (key == NULL) {
    lc_error_set(error, LC_ERR_NOMEM, 0L,
...
lc_pouch_txn_collect_attachment_staged(const lc_pouch_state_visit_entry *entry,
  ...
  if (key_len <= collect->prefix_len + collect->suffix_len ||
      strcmp(entry->key + key_len - collect->suffix_len, collect->suffix) !=
          0) {
    return LC_OK;
  }
...
static int
lc_pouch_txn_collect_queue_staged(const lc_pouch_state_visit_entry *entry,
                                  void *context, lc_error *error) {
...
static char *lc_pouch_txn_staging_suffix(const char *txn_id, lc_error *error) {
  char *suffix;
  size_t suffix_len;
  size_t txn_len;
  ...
  if (!lc_pouch_txn_id_present(txn_id)) {
    (void)lc_error_set(error, LC_ERR_INVALID, 0L,
                       "pouch transaction staging suffix requires txn_id", NULL,
                       NULL, NULL);
    return NULL;
  }
  suffix_len = strlen("/.staging/");
  txn_len = strlen(txn_id);
  suffix = (char *)lc_alloc_with_allocator(NULL, suffix_len + txn_len + 1U);
  if (suffix == NULL) {
    (void)lc_error_set(error, LC_ERR_NOMEM, 0L,
...
static int lc_pouch_collect_staged_attachment_bases(
  ...
  lc_free_with_allocator(NULL, suffix);
  lc_free_with_allocator(NULL, prefix);
  return rc;
}
...
static int lc_pouch_collect_staged_queue_bases(lc_client_handle *client,
  ...
  rc = lc_pouch_state_visit(client->pouch, ".lockd/queue",
                            lc_pouch_txn_collect_queue_staged, &collect, error);
  if (rc == LC_OK) {
    *out = collect.base_keys;
    memset(&collect.base_keys, 0, sizeof(collect.base_keys));
  }
...
static int lc_pouch_txn_apply_attachment_participant(
  ...
  for (i = 0U; rc == LC_OK && i < base_keys.count; ++i) {
    if (strcmp(state, "commit") == 0) {
      ...
      rc = lc_pouch_state_discard_staged(client->pouch, ".lockd/attachments",
                                         base_keys.keys[i], txn_id, &discarded,
                                         error);
    }
  }
...
static int lc_pouch_txn_commit_queue_stage(lc_client_handle *client,
  ...
  rc = lc_pouch_state_read(client->pouch, ".lockd/queue", staged_key, &staged,
                           error);
  if (rc == LC_OK && staged.found) {
...
static int lc_pouch_acquire_locked(void *context, lc_error *error) {
  ...
  unsigned long expected_lease_version;
  long now_seconds;
  int rc;
  ...
  rc = lc_pouch_state_read_metadata_locked(
      ctx->client->pouch, ctx->namespace_name, ctx->req->key, &ctx->read_result,
      error);
  if (rc != LC_OK) {
    return rc;
  }
  if (ctx->req->if_not_exists && ctx->read_result.found) {
    return lc_error_set(error, LC_ERR_INVALID, 0L,
  ...
  rc = lc_pouch_write_lease_record_with_lock_state(
      ...
      ctx->lease_expires_at_unix, expected_lease_version,
      1, &ctx->lease_write_result, error);
  if (rc != LC_OK) {
...
int lc_pouch_client_acquire_method(lc_client *self, const lc_acquire_req *req,
  ...
  *out = NULL;
  ...
  now_seconds = (long)time(NULL);
  block_deadline_unix =
      req->block_seconds > 0L ? now_seconds + req->block_seconds : now_seconds;
  for (;;) {
  ...
  lease = lc_lease_new(
      ...
      req->txn_id, acquire_context.fencing_token, (long)acquire_context.version,
...
int lc_pouch_client_remove_method(lc_client *self, const lc_remove_op *req,
  ...
  if (req->has_if_version) {
    if (req->if_version < 0L) {
      return lc_error_set(error, LC_ERR_INVALID, 0L,
                          "pouch remove if_version must be non-negative", NULL,
                          NULL, NULL);
    }
    options.expected_version = (unsigned long)req->if_version;
    options.has_expected_version = 1;
...
int lc_pouch_client_get_attachment_method(lc_client *self,
  ...
  rc = lc_pouch_state_read(client->pouch, ".lockd/attachments", attachment_key,
                           &read_result, error);
  if (rc == LC_OK && !read_result.found) {
  ...
  *deleted = 0;
  ...
  if (rc == LC_OK && lc_pouch_txn_id_present(req->lease.txn_id)) {
    ...
    rc = lc_pouch_state_delete(client->pouch, ".lockd/attachments",
                               attachment_key, &options, &write_result, error);
    if (rc == LC_OK) {
      *deleted = 1;
    }
  }
...
int lc_pouch_client_enqueue_method(lc_client *self, const lc_enqueue_req *req,
  ...
  lc_client_handle *client;
  lc_pouch_queue_record record;
  lc_pouch_queue_stream_source_ctx *stream_context;
  ...
  if (rc == LC_OK && stream_context != NULL) {
    record.payload_length = stream_context->payload_bytes;
  }
  if (rc == LC_OK) {
...
int lc_pouch_client_dequeue_with_state_method(lc_client *self,
  ...
  *out = NULL;
  ...
  state_key =
      lc_pouch_queue_state_key(handle->queue, handle->message_id, error);
  if (state_key == NULL) {
    rc = error != NULL && error->code != LC_OK ? error->code : LC_ERR_NOMEM;
    goto cleanup;
  }
...
int lc_pouch_client_dequeue_batch_method(lc_client *self,
  ...
  if (req->queue == NULL || req->queue[0] == '\0') {
    return lc_error_set(error, LC_ERR_INVALID, 0L,
                        "pouch dequeue_batch requires queue", NULL, NULL,
                        NULL);
  }
  page_req = *req;
...
int lc_pouch_client_update_namespace_config_method(
  ...
  if (rc == LC_OK) {
    rc =
        lc_pouch_state_write(client->pouch, LC_POUCH_NAMESPACE_CONFIG_NAMESPACE,
                             key, source, &options, &write_result, error);
  }
  if (source != NULL) {
    lc_source_close(source);
  }
...
static int lc_pouch_client_txn_decision(lc_client *self,
  ...
  if (rc == LC_OK) {
    rc = lc_pouch_txn_decision_response(out, req->txn_id, state, result.version,
                                        error);
  }
  lc_pouch_state_write_result_cleanup(&client->allocator, &result);
  lc_pouch_txn_buffer_cleanup(&record);
...
static int lc_pouch_tc_read_lease(lc_client_handle *client,
  ...
  body = NULL;
  body_length = 0U;
  rc = lc_pouch_state_read(client->pouch, ".lockd/tc", "leader", &read_result,
                           error);
  if (rc == LC_OK && read_result.found) {
...
static int lc_pouch_tc_write_endpoint(lc_client_handle *client,
                                      const char *namespace_name,
                                      const char *key, const char *endpoint,
                                      lc_error *error) {
  lc_pouch_state_write_options options;
  lc_pouch_state_write_result result;
  lc_source *source;
...
static char *lc_pouch_tc_rm_key(const char *backend_hash, const char *endpoint,
                                lc_error *error) {
  char *backend_hex;
  char *endpoint_hex;
  char *key;
  ...
  if (backend_hex == NULL || endpoint_hex == NULL) {
    lc_free_with_allocator(NULL, backend_hex);
    lc_free_with_allocator(NULL, endpoint_hex);
    lc_error_set(error, LC_ERR_NOMEM, 0L, "failed to allocate pouch TC RM key",
                 NULL, NULL, NULL);
    return NULL;
  }
  key = (char *)lc_alloc_with_allocator(NULL, strlen("backend//endpoint/") +
                                                  strlen(backend_hex) +
...
static int lc_pouch_tc_rm_scan_visit(const lc_pouch_state_visit_entry *entry,
  ...
  return lc_pouch_tc_endpoint_list_append(&scan->endpoints, entry->content_type,
                                          error);
}
...
static int lc_pouch_tc_rm_res_response(lc_client_handle *client,
  ...
  prefix = (char *)lc_alloc_with_allocator(NULL, strlen("backend//endpoint/") +
                                                     strlen(backend_hex) + 1U);
  if (prefix == NULL) {
    lc_free_with_allocator(NULL, backend_hex);
    return lc_error_set(error, LC_ERR_NOMEM, 0L,
...
static int lc_pouch_tc_string_list_append(lc_string_list *list,
  ...
  if (list->items[list->count] == NULL) {
    return lc_error_set(error, LC_ERR_NOMEM, 0L,
                        "failed to allocate pouch TC string", NULL, NULL, NULL);
  }
  ++list->count;
  return LC_OK;
}

static lc_tc_rm_backend *lc_pouch_tc_rm_list_backend(lc_tc_rm_list_res *out,
                                                     const char *backend_hash,
                                                     lc_error *error) {
  lc_tc_rm_backend *backends;
  size_t i;
  ...
  backends = (lc_tc_rm_backend *)lc_realloc_with_allocator(
      NULL, out->backends,
      (out->backend_count + 1U) * sizeof(lc_tc_rm_backend));
  if (backends == NULL) {
    lc_error_set(error, LC_ERR_NOMEM, 0L,
...
static int lc_pouch_tc_rm_list_visit(const lc_pouch_state_visit_entry *entry,
  ...
  backend_hex = (char *)lc_alloc_with_allocator(NULL, backend_hex_length + 1U);
  if (backend_hex == NULL) {
    return lc_error_set(error, LC_ERR_NOMEM, 0L,
                        "failed to allocate pouch TC RM backend key", NULL,
                        NULL, NULL);
...
int lc_pouch_client_tc_lease_release_method(lc_client *self,
  ...
  if (rc == LC_OK && record.found && record.expires_at_unix > now &&
      ...
      strcmp(record.leader_id, req->leader_id) == 0) {
    released = 1;
    rc = lc_pouch_tc_delete_key(client, ".lockd/tc", "leader", 1,
...
int lc_pouch_client_tc_cluster_leave_method(lc_client *self,
  ...
  client = (lc_client_handle *)self;
  rc = lc_pouch_tc_delete_key(client, ".lockd/tc-cluster", "self", 0, 0UL,
                              error);
  if (rc == LC_OK) {
    rc = lc_pouch_tc_cluster_response(client, out, error);
  }
...
int lc_pouch_client_tc_rm_unregister_method(lc_client *self,
  ...
  rc = lc_pouch_tc_delete_key(client, ".lockd/tc-rm", key, 0, 0UL, error);
  if (rc == LC_OK) {
    rc = lc_pouch_tc_rm_res_response(client, req->backend_hash, out, error);
  }
  lc_free_with_allocator(NULL, key);
...
static int lc_pouch_lease_validate_private_read(lc_lease_handle *lease,
  ...
  return lc_pouch_validate_lease_record(lease->client, &ref,
                                        lease->namespace_name, lease->key, NULL,
                                        error);
}
...
static int lc_pouch_lease_staged_update_method(lc_lease *self, lc_source *src,
  ...
  if (rc == LC_OK) {
    ...
    rc = lc_pouch_lease_refresh_state(lease, result.etag, (long)result.version,
                                      error);
  }
  lc_pouch_state_write_result_cleanup(&lease->client->allocator, &result);
  return rc;
