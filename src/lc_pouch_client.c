...
typedef struct lc_pouch_lql_document_filter {
  ...
  lc_pouch_document_date_after_term *document_fast_date_after_terms;
  size_t document_fast_date_after_term_count;
  int enabled;
...
lc_pouch_lql_document_filter_cleanup(lc_pouch_lql_document_filter *filter) {
  ...
  for (index = 0U; index < filter->document_fast_date_after_term_count;
       ++index) {
    lc_free_with_allocator(
        NULL, (char *)filter->document_fast_date_after_terms[index].field);
    lc_free_with_allocator(
        NULL, (char *)filter->document_fast_date_after_terms[index].after);
  }
  lc_free_with_allocator(NULL, filter->document_fast_date_after_terms);
...
static int lc_pouch_lql_view_absent(lql_string_view view) {
...
static int lc_pouch_lql_ast_fast_date_after_parse(
  ...
  if (filter == NULL || filter->runtime == NULL || filter->selector == NULL ||
      filter->document_fast_date_after_term_count > 0U) {
    return 0;
  }
  memset(&root, 0, sizeof(root));
  lql_error_init(&lql_err);
  if (filter->runtime->selector_root(filter->runtime, filter->selector, &root,
                                     &lql_err) != LQL_STATUS_OK ||
      root.kind != LQL_SELECTOR_NODE_DATE) {
    return 0;
  }
  memset(&source, 0, sizeof(source));
  lql_error_init(&lql_err);
  if (filter->runtime->selector_node_date_term(filter->runtime, root, &source,
                                               &lql_err) != LQL_STATUS_OK ||
      !lc_pouch_lql_ast_view_is_pointer(source.field) ||
      !lc_pouch_lql_view_absent(source.value) ||
      !lc_pouch_lql_view_absent(source.since) ||
      !lc_pouch_lql_view_absent(source.before) ||
      !lc_pouch_lql_view_absent(source.gte) ||
      !lc_pouch_lql_view_absent(source.lt) ||
      !lc_pouch_lql_view_absent(source.lte)) {
    return 0;
  }
  after = !lc_pouch_lql_view_absent(source.after) ? source.after : source.gt;
  if (!lc_pouch_lql_ast_view_is_cstr(after) || after.len == 0U ||
      (!lc_pouch_lql_view_absent(source.after) &&
       !lc_pouch_lql_view_absent(source.gt))) {
    return 0;
  }
  term = (lc_pouch_document_date_after_term *)lc_calloc_with_allocator(
      NULL, 1U, sizeof(term[0]));
  if (term == NULL) {
    return 0;
  }
  term->field = lc_pouch_lql_ast_view_copy(source.field);
  ...
  filter->document_fast_date_after_terms = term;
  filter->document_fast_date_after_term_count = 1U;
  return 1;
}
...
lc_pouch_lql_document_filter_init(lc_pouch_lql_document_filter *filter,
  ...
  if (capabilities.wildcard_path || capabilities.recursive_path) {
    ...
    (void)lc_pouch_lql_ast_fast_date_after_parse(filter);
...
static lonejson_status
lc_pouch_lql_fast_date_string_begin(void *user, const lonejson_value_path *path,
                                    lonejson_error *error) {
  lc_pouch_lql_fast_match_visit *visit;
...
static lonejson_status
lc_pouch_lql_fast_date_string_chunk(void *user, const lonejson_value_path *path,
                                    const char *data, size_t len,
                                    lonejson_error *error) {
  lc_pouch_lql_fast_match_visit *visit;
...
lc_pouch_lql_fast_date_string_end(void *user, const lonejson_value_path *path,
  ...
  (void)error;
  visit = (lc_pouch_lql_fast_match_visit *)user;
  if (visit == NULL || visit->filter == NULL || visit->field == NULL ||
      visit->value == NULL ||
      visit->filter->document_fast_date_after_term_count != 1U) {
    return LONEJSON_STATUS_INVALID_ARGUMENT;
  }
  term = &visit->filter->document_fast_date_after_terms[0];
...
static int lc_pouch_lql_document_filter_match_fast_date_bytes(
  ...
  if (handled == NULL || matched == NULL || filter == NULL ||
      filter->document_fast_date_after_term_count != 1U) {
    return LC_OK;
  }
  runtime = lc_thread_lonejson_runtime();
  ...
  visit.filter = filter;
  visitor = lonejson_default_path_value_visitor();
  visitor.string_begin = lc_pouch_lql_fast_date_string_begin;
  visitor.string_chunk = lc_pouch_lql_fast_date_string_chunk;
  visitor.string_end = lc_pouch_lql_fast_date_string_end;
  lonejson_error_init(&lj_error);
  ...
  if (visit.fast_date_unsupported) {
    return LC_OK;
  }
  *handled = 1;
  *matched = visit.fast_date_matched;
  return LC_OK;
}
...
static int lc_pouch_lql_document_filter_match_bytes(
  ...
  handled = 0;
  rc = lc_pouch_lql_document_filter_match_fast_date_bytes(
...
static size_t lc_pouch_query_json_escaped_len(const char *value,
                                              int *overflow) {
  size_t length;
  const unsigned char *cursor;
...
static int lc_pouch_query_size_add(size_t *total, size_t add) {
...
static void lc_pouch_query_json_append_value(char *dst, size_t *offset,
                                             const char *value) {
  ...
  while (*cursor != '\0') {
    ...
    default:
      if (ch < 0x20U) {
        ...
        dst[pos++] = '0';
        dst[pos++] = '0';
        dst[pos++] = hex[(ch >> 4) & 0x0FU];
        dst[pos++] = hex[ch & 0x0FU];
      } else {
        dst[pos++] = (char)ch;
      }
      break;
...
static int lc_pouch_query_row_prefix_len(const char *key,
  ...
  if (key == NULL || version_text == NULL || version_capacity == 0U ||
      prefix_len == NULL) {
    return lc_error_set(error, LC_ERR_INVALID, 0L,
                        "pouch query row prefix requires key and output", NULL,
                        NULL, NULL);
  }
  ...
  overflow = 0;
  key_len = lc_pouch_query_json_escaped_len(key, &overflow);
  content_type_len = lc_pouch_query_json_escaped_len(content_type, &overflow);
  etag_len = lc_pouch_query_json_escaped_len(etag, &overflow);
  if (overflow) {
    return lc_error_set(error, LC_ERR_INVALID, 0L,
                        "pouch query row metadata is too large", NULL, NULL,
                        NULL);
  }
  total = 0U;
  if (!lc_pouch_query_size_add(&total, sizeof(key_field) - 1U) ||
      !lc_pouch_query_size_add(&total, key_len) ||
      !lc_pouch_query_size_add(&total, sizeof(content_type_field) - 1U) ||
      !lc_pouch_query_size_add(&total, content_type_len) ||
      !lc_pouch_query_size_add(&total, sizeof(etag_field) - 1U) ||
      !lc_pouch_query_size_add(&total, etag_len) ||
      !lc_pouch_query_size_add(&total, sizeof(version_field) - 1U) ||
      !lc_pouch_query_size_add(&total, version_len)) {
    return lc_error_set(error, LC_ERR_INVALID, 0L,
                        "pouch query row metadata is too large", NULL, NULL,
                        NULL);
  }
...
static int lc_pouch_query_format_row_prefix(char *dst, size_t capacity,
                                            const char *key,
                                            const lc_pouch_state_info *state,
                                            size_t *prefix_len,
                                            lc_error *error) {
  ...
  rc = lc_pouch_query_row_prefix_len(key, state, version_text,
                                     sizeof(version_text), prefix_len, error);
  if (rc != LC_OK || *prefix_len > capacity) {
    return rc;
  }
  content_type = state != NULL ? state->content_type : NULL;
  etag = state != NULL ? state->etag : NULL;
  offset = 0U;
  memcpy(dst + offset, key_field, sizeof(key_field) - 1U);
  offset += sizeof(key_field) - 1U;
  lc_pouch_query_json_append_value(dst, &offset, key);
  memcpy(dst + offset, content_type_field, sizeof(content_type_field) - 1U);
  offset += sizeof(content_type_field) - 1U;
  lc_pouch_query_json_append_value(dst, &offset, content_type);
  memcpy(dst + offset, etag_field, sizeof(etag_field) - 1U);
  offset += sizeof(etag_field) - 1U;
  lc_pouch_query_json_append_value(dst, &offset, etag);
  memcpy(dst + offset, version_field, sizeof(version_field) - 1U);
  offset += sizeof(version_field) - 1U;
  memcpy(dst + offset, version_text, strlen(version_text));
  offset += strlen(version_text);
  *prefix_len = offset;
  return LC_OK;
}
...
static int lc_pouch_query_sink_write_json_value(lc_sink *dst,
  ...
  if (value == NULL) {
    return lc_pouch_sink_write_all(dst, "null", 4U, error);
  }
  if (lc_pouch_sink_write_all(dst, "\"", 1U, error) != LC_OK) {
    return error != NULL && error->code != LC_OK ? error->code
                                                 : LC_ERR_TRANSPORT;
  }
  cursor = (const unsigned char *)value;
  while (*cursor != '\0') {
    ...
    if (two != NULL) {
      if (lc_pouch_sink_write_all(dst, two, 2U, error) != LC_OK) {
        return error != NULL && error->code != LC_OK ? error->code
                                                     : LC_ERR_TRANSPORT;
      }
    } else if (ch < 0x20U) {
      ...
      escape[2] = '0';
      escape[3] = '0';
      escape[4] = hex[(ch >> 4) & 0x0FU];
      escape[5] = hex[ch & 0x0FU];
      escape[6] = '\0';
      if (lc_pouch_sink_write_all(dst, escape, 6U, error) != LC_OK) {
        return error != NULL && error->code != LC_OK ? error->code
                                                     : LC_ERR_TRANSPORT;
      }
    } else if (lc_pouch_sink_write_all(dst, &ch, 1U, error) != LC_OK) {
      return error != NULL && error->code != LC_OK ? error->code
                                                   : LC_ERR_TRANSPORT;
    }
  }
  return lc_pouch_sink_write_all(dst, "\"", 1U, error);
}
...
static int lc_pouch_query_write_row_prefix(lc_sink *dst, const char *key,
  ...
  static const char etag_field[] = ",\"etag\":";
  static const char version_field[] = ",\"version\":";
  static const char document_field[] = ",\"document\":";
  char prefix[512];
  char version_text[64];
  const char *content_type;
  const char *etag;
  ...
  rc = lc_pouch_query_format_row_prefix(prefix, sizeof(prefix), key, state,
                                        &prefix_len, error);
  if (rc != LC_OK) {
    return rc;
  }
  if (prefix_len <= sizeof(prefix)) {
    rc = lc_pouch_sink_write_all(dst, prefix, prefix_len, error);
    if (rc != LC_OK) {
      return rc;
    }
    return lc_pouch_sink_write_all(dst, document_field,
                                   sizeof(document_field) - 1U, error);
  }
  content_type = state != NULL ? state->content_type : NULL;
  etag = state != NULL ? state->etag : NULL;
  written = snprintf(version_text, sizeof(version_text), "%ld",
  ...
  rc = lc_pouch_sink_write_all(dst, key_field, sizeof(key_field) - 1U, error);
  if (rc == LC_OK) {
    rc = lc_pouch_query_sink_write_json_value(dst, key, error);
  }
  if (rc == LC_OK) {
    rc = lc_pouch_sink_write_all(dst, content_type_field,
                                 sizeof(content_type_field) - 1U, error);
  }
  if (rc == LC_OK) {
    rc = lc_pouch_query_sink_write_json_value(dst, content_type, error);
  }
  if (rc == LC_OK) {
    rc = lc_pouch_sink_write_all(dst, etag_field, sizeof(etag_field) - 1U,
                                 error);
  }
  if (rc == LC_OK) {
...
static int lc_pouch_query_try_write_small_json_row(
    lc_pouch_query_scan_context *scan, const char *key,
    const lc_pouch_state_info *state, lc_source *body, int *handled,
    lc_error *error) {
  static const char document_field[] = ",\"document\":";
  unsigned char row_buffer[8192];
  size_t prefix_len;
  size_t body_len;
  size_t total_len;
  size_t offset;
  ...
  if (scan == NULL || key == NULL || state == NULL || body == NULL ||
      handled == NULL || state->bytes < 0L) {
    return LC_OK;
  }
  body_len = (size_t)state->bytes;
  rc = lc_pouch_query_format_row_prefix((char *)row_buffer, sizeof(row_buffer),
                                        key, state, &prefix_len, error);
  if (rc != LC_OK) {
    return rc;
  }
  if (prefix_len > sizeof(row_buffer)) {
    return LC_OK;
  }
  total_len = prefix_len;
  if (!lc_pouch_query_size_add(&total_len, sizeof(document_field) - 1U) ||
      !lc_pouch_query_size_add(&total_len, body_len) ||
      !lc_pouch_query_size_add(&total_len, 2U)) {
    return LC_OK;
  }
  if (total_len > sizeof(row_buffer)) {
    return LC_OK;
  }
  offset = prefix_len;
  memcpy(row_buffer + offset, document_field, sizeof(document_field) - 1U);
  offset += sizeof(document_field) - 1U;
  while (offset < total_len - 2U) {
    size_t got;

    got = body->read(body, row_buffer + offset, total_len - 2U - offset, error);
    if (got == 0U) {
      if (error != NULL && error->code != LC_OK) {
        return error->code;
      }
  ...
  if (offset != total_len - 2U) {
    return lc_error_set(error, LC_ERR_TRANSPORT, 0L,
                        "pouch query row body ended early", NULL, NULL, NULL);
  }
  row_buffer[offset++] = '}';
  row_buffer[offset++] = '\n';
  rc = lc_pouch_sink_write_all(scan->dst, row_buffer, offset, error);
  if (rc == LC_OK) {
    *handled = 1;
  }
  return rc;
...
static int lc_pouch_query_scan_write_row(lc_pouch_query_scan_context *scan,
  ...
  if (embed_json) {
    ...
    handled = 0;
    rc = lc_pouch_query_try_write_small_json_row(scan, row->key, state_view,
                                                 body, &handled, error);
    if (rc != LC_OK || handled) {
      if (body != NULL && !borrowed_body) {
        body->close(body);
...
static int lc_pouch_client_query_index(lc_client_handle *client,
  ...
  scan_req.document_or_exists_path_pattern_count =
      filter.document_or_exists_path_pattern_count;
  scan_req.document_date_after_terms = filter.document_fast_date_after_terms;
  scan_req.document_date_after_term_count =
      filter.document_fast_date_after_term_count;
  visit.scan.client = client;
  visit.scan.dst = dst;
