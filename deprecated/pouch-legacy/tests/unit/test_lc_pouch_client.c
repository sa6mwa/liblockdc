...
static void test_query_exact_generation_path(const char *root,
                                             const char *namespace_name,
                                             char *path, size_t path_size) {
  snprintf(path, path_size, "%s/%%2elockd/logstore/query.index.exact/%s.lcpttg",
           root, namespace_name);
}
...
static void test_query_number_generation_path(const char *root,
                                              const char *namespace_name,
                                              char *path, size_t path_size) {
  snprintf(path, path_size,
           "%s/%%2elockd/logstore/query.index.number/%s.lcpngn", root,
           namespace_name);
}

static void test_query_text_generation_path(const char *root,
                                            const char *namespace_name,
                                            char *path, size_t path_size) {
  snprintf(path, path_size, "%s/%%2elockd/logstore/query.index.text/%s.lcptxg",
           root, namespace_name);
}
...
static void
test_read_exact_generation_file(const char *path,
                                lc_pouch_index_term_generation *out) {
  struct stat st;
  unsigned char *bytes;
  size_t offset;
  int fd;
  ...
  assert_int_equal(close(fd), 0);
  assert_true(lc_pouch_index_term_generation_decode(NULL, out, bytes,
                                                    (size_t)st.st_size));
  free(bytes);
}

static void
test_read_number_generation_file(const char *path,
                                 lc_pouch_index_number_generation *out) {
  struct stat st;
  unsigned char *bytes;
  size_t offset;
  int fd;
  ...
  assert_int_equal(close(fd), 0);
  assert_true(lc_pouch_index_number_generation_decode(NULL, out, bytes,
                                                      (size_t)st.st_size));
  free(bytes);
}

static void
test_read_text_generation_file(const char *path,
                               lc_pouch_index_text_generation *out) {
  struct stat st;
  unsigned char *bytes;
  size_t offset;
  int fd;
  ...
  assert_int_equal(close(fd), 0);
  assert_true(lc_pouch_index_text_generation_decode(NULL, out, bytes,
                                                    (size_t)st.st_size));
  free(bytes);
}
...
static void test_corrupt_doc_generation_file(const char *path) {
  static const char corrupt[] = "not-a-doc-generation";
  int fd;
...
static void test_corrupt_exact_generation_file(const char *path) {
  static const char corrupt[] = "not-an-exact-generation";
  int fd;
...
static void test_corrupt_number_generation_file(const char *path) {
  static const char corrupt[] = "not-a-number-generation";
  int fd;
...
static void test_corrupt_text_generation_file(const char *path) {
  static const char corrupt[] = "not-a-text-generation";
  int fd;
...
static void test_stale_exact_generation_file(const char *path) {
  ...
  memset(&generation, 0, sizeof(generation));
  test_read_exact_generation_file(path, &generation);
  assert_true(generation.identity.sequence > 0U);
  generation.identity.sequence--;
  assert_true(
      lc_pouch_index_term_generation_encoded_size(&generation, &encoded_size));
  encoded = (unsigned char *)malloc(encoded_size);
  assert_non_null(encoded);
  assert_true(lc_pouch_index_term_generation_encode(&generation, encoded,
...
static void test_stale_number_generation_file(const char *path) {
  ...
  memset(&generation, 0, sizeof(generation));
  test_read_number_generation_file(path, &generation);
  assert_true(generation.identity.sequence > 0U);
  generation.identity.sequence--;
  assert_true(lc_pouch_index_number_generation_encoded_size(&generation,
                                                            &encoded_size));
  encoded = (unsigned char *)malloc(encoded_size);
  assert_non_null(encoded);
  assert_true(lc_pouch_index_number_generation_encode(&generation, encoded,
                                                      encoded_size, &written));
  assert_int_equal(written, encoded_size);
  fd = open(path, O_WRONLY | O_TRUNC);
  assert_true(fd >= 0);
...
static void test_stale_text_generation_file(const char *path) {
  ...
  memset(&generation, 0, sizeof(generation));
  test_read_text_generation_file(path, &generation);
  assert_true(generation.identity.sequence > 0U);
  generation.identity.sequence--;
  assert_true(
...
static void test_pouch_endpoint_scan_query_filters_key_selector(void **state) {
  char root[256];
  char endpoint[320];
  lc_client *client;
  lc_lease *alpha;
  ...
  (void)state;
  test_root_path(root, sizeof(root), "query-scan-key");
  test_cleanup_root(root);
  test_endpoint(endpoint, sizeof(endpoint), root);
  memset(&error, 0, sizeof(error));
  memset(&res, 0, sizeof(res));
  ...
  assert_int_equal(rc, LC_OK);
  lc_query_req_init(&req);
  req.selector_json = "{\"key\":\"bravo\",\"owner\":\"owner-b\"}";
  req.engine = "scan";
  rc = client->query(client, &req, sink, &res, &error);
  assert_int_equal(rc, LC_OK);
  text = memory_sink_text(sink);
  ...
  assert_int_equal(rc, LC_OK);
  lc_query_req_init(&req);
  req.selector_json = "{\"key\":\"bravo\",\"owner\":\"owner-a\"}";
  req.engine = "scan";
  rc = client->query(client, &req, sink, &res, &error);
  assert_int_equal(rc, LC_OK);
  text = memory_sink_text(sink);
  ...
  assert_null(res.cursor);
  assert_string_equal(res.metadata_json, "{\"query_candidates\":0}");
  assert_int_equal(res.index_seq, 0UL);
  free(text);
  lc_sink_close(sink);
  lc_query_res_cleanup(&res);
...
static void test_pouch_endpoint_query_filters_full_form_lql_document_selector(
  ...
  (void)state;
  test_root_path(root, sizeof(root), "query-full-form-lql");
  test_cleanup_root(root);
  test_endpoint(endpoint, sizeof(endpoint), root);
  memset(&error, 0, sizeof(error));
  memset(&res, 0, sizeof(res));
  ...
  assert_int_equal(rc, LC_OK);
  lc_query_req_init(&req);
  req.selector_json = "{\"or\":[{\"date\":{\"field\":\"/created_at\","
                      "\"after\":\"2025-01-01T00:00:00Z\"}},"
                      "{\"eq\":{\"field\":\"/value\",\"value\":\"beta\"}}]}";
  rc = client->query(client, &req, sink, &res, &error);
  assert_int_equal(rc, LC_OK);
  text = memory_sink_text(sink);
  assert_non_null(strstr(text, "{\"key\":\"alpha\""));
  ...
  lc_query_req_init(&req);
  req.selector_json =
      "{\"or\":[{\"eq\":{\"field\":\"/value\",\"value\":\"alpha\"}},"
      "{\"or\":[{\"exists\":\"/other\"},"
      "{\"prefix\":{\"field\":\"/value\",\"value\":\"zz\"}}]}]}";
  rc = client->query(client, &req, sink, &res, &error);
  assert_int_equal(rc, LC_OK);
  text = memory_sink_text(sink);
  assert_non_null(strstr(text, "{\"key\":\"alpha\""));
  ...
  lc_query_req_init(&req);
  req.selector_json =
      "{\"and\":[{\"and\":[{\"or\":[{\"eq\":{\"field\":\"/value\","
      "\"value\":\"alpha\"}},{\"eq\":{\"field\":\"/n\",\"value\":4}}]},"
      "{\"not\":{\"exists\":\"/missing\"}}]}]}";
  req.engine = "scan";
  rc = client->query(client, &req, sink, &res, &error);
  assert_int_equal(rc, LC_OK);
  ...
  req.selector_json =
      "{\"and\":[{\"and\":[{\"or\":[{\"eq\":{\"field\":\"/value\","
      "\"value\":\"alpha\"}},{\"range\":{\"field\":\"/n\",\"gt\":3}},"
      "{\"contains\":{\"field\":\"/kind\",\"value\":\"select\"}}]},"
      "{\"not\":{\"exists\":\"/missing\"}}]}]}";
  req.engine = "scan";
  rc = client->query(client, &req, sink, &res, &error);
  assert_int_equal(rc, LC_OK);
  ...
  lc_query_req_init(&req);
  req.selector_json =
      "{\"and\":[{\"or\":[{\"in\":{\"field\":\"/value\","
      "\"any\":[\"alpha\"]}},{\"range\":{\"field\":\"/n\",\"gt\":3}}]},"
      "{\"or\":[{\"exists\":\"/box\"},"
      "{\"contains\":{\"field\":\"/kind\",\"value\":\"oth\"}}]}]}";
  req.engine = "scan";
  rc = client->query(client, &req, sink, &res, &error);
  assert_int_equal(rc, LC_OK);
  text = memory_sink_text(sink);
  ...
  req.selector_json =
      "{\"or\":[{\"in\":{\"field\":\"/value\",\"any\":[\"alpha\"]}},"
      "{\"icontains\":{\"field\":\"/kind\",\"value\":\"OTH\"}}]}";
  req.engine = "scan";
  rc = client->query(client, &req, sink, &res, &error);
  assert_int_equal(rc, LC_OK);
  text = memory_sink_text(sink);
  ...
  assert_int_equal(rc, LC_OK);
  lc_query_req_init(&req);
  req.selector_json = "{\"and\":[{\"and\":[{\"eq\":{\"field\":\"/value\","
                      "\"value\":\"beta\"}}]}]}";
  rc = client->query(client, &req, sink, &res, &error);
  assert_int_equal(rc, LC_OK);
  text = memory_sink_text(sink);
  ...
  assert_int_equal(rc, LC_OK);
  lc_query_req_init(&req);
  req.selector_json = "{\"and\":[{\"and\":[{\"eq\":{\"field\":\"/n\","
                      "\"value\":1.0}}]}]}";
  rc = client->query(client, &req, sink, &res, &error);
  assert_int_equal(rc, LC_OK);
  text = memory_sink_text(sink);
  ...
  assert_int_equal(rc, LC_OK);
  lc_query_req_init(&req);
  req.selector_json = "{\"or\":[{\"range\":{\"field\":\"/n\",\"lt\":2}},"
                      "{\"prefix\":{\"field\":\"/other\",\"value\":\"al\"}}]}";
  req.engine = "scan";
  rc = client->query(client, &req, sink, &res, &error);
  assert_int_equal(rc, LC_OK);
  ...
  lc_query_req_init(&req);
  req.selector_json =
      "{\"or\":[{\"prefix\":{\"field\":\"/other\",\"value\":\"al\"}},"
      "{\"icontains\":{\"field\":\"/kind\",\"value\":\"OTH\"}}]}";
  req.engine = "scan";
  rc = client->query(client, &req, sink, &res, &error);
  assert_int_equal(rc, LC_OK);
  ...
  req.selector_json =
      "{\"or\":[{\"exists\":\"/box\"},"
      "{\"icontains\":{\"field\":\"/value\",\"value\":\"ET\"}}]}";
  req.engine = "scan";
  rc = client->query(client, &req, sink, &res, &error);
  assert_int_equal(rc, LC_OK);
  text = memory_sink_text(sink);
  ...
  assert_int_equal(rc, LC_OK);
  lc_query_req_init(&req);
  req.selector_json = "{\"or\":[{\"exists\":\"/box\"},"
                      "{\"prefix\":{\"field\":\"/value\",\"value\":\"be\"}}]}";
  req.engine = "scan";
  rc = client->query(client, &req, sink, &res, &error);
  assert_int_equal(rc, LC_OK);
  text = memory_sink_text(sink);
  ...
  assert_int_equal(rc, LC_OK);
  lc_query_req_init(&req);
  req.selector_json = "{\"iprefix\":{\"field\":\"/value\",\"value\":\"AL\"}}";
  rc = client->query(client, &req, sink, &res, &error);
  assert_int_equal(rc, LC_OK);
  text = memory_sink_text(sink);
  assert_non_null(strstr(text, "{\"key\":\"alpha\""));
  ...
  assert_int_equal(rc, LC_OK);
  lc_query_req_init(&req);
  req.selector_json = "{\"exists\":\"/value\"}";
  req.engine = "scan";
  rc = client->query(client, &req, sink, &res, &error);
  assert_int_equal(rc, LC_OK);
  text = memory_sink_text(sink);
  ...
  assert_int_equal(rc, LC_OK);
  lc_query_req_init(&req);
  req.selector_json = "{\"range\":{\"field\":\"/n\",\"gt\":1,\"lt\":4}}";
  req.engine = "scan";
  rc = client->query(client, &req, sink, &res, &error);
  assert_int_equal(rc, LC_OK);
  text = memory_sink_text(sink);
  ...
  assert_int_equal(rc, LC_OK);
  lc_query_req_init(&req);
  req.selector_json = "{\"eq\":{\"field\":\"/value\",\"value\":\"beta\"}}";
  req.engine = "scan";
  req.limit = 1L;
  rc = client->query(client, &req, sink, &res, &error);
  assert_int_equal(rc, LC_OK);
  ...
  req.selector_json =
      "{\"and\":[{\"exists\":\"/value\"},"
      "{\"not\":{\"prefix\":{\"field\":\"/value\",\"value\":\"be\"}}}]}";
  rc = client->query(client, &req, sink, &res, &error);
  assert_int_equal(rc, LC_OK);
  text = memory_sink_text(sink);
  assert_non_null(strstr(text, "{\"key\":\"alpha\""));
  ...
  req.selector_json =
      "{\"and\":[{\"range\":{\"field\":\"/n\",\"gt\":1,\"lt\":4}},"
      "{\"not\":{\"contains\":{\"field\":\"/kind\",\"value\":\"select\"}}}]}";
  rc = client->query(client, &req, sink, &res, &error);
  assert_int_equal(rc, LC_OK);
  text = memory_sink_text(sink);
  assert_null(strstr(text, "{\"key\":\"alpha\""));
  ...
  req.selector_json =
      ...
      "{\"or\":[{\"eq\":{\"field\":\"/value\",\"value\":\"beta\"}},"
      "{\"exists\":\"/box\"}]},"
      "{\"or\":[{\"range\":{\"field\":\"/n\",\"lt\":3}},"
      "{\"prefix\":{\"field\":\"/kind\",\"value\":\"oth\"}}]},"
      "{\"not\":{\"contains\":{\"field\":\"/kind\",\"value\":\"other\"}}}"
      "]}";
  rc = client->query(client, &req, sink, &res, &error);
  assert_int_equal(rc, LC_OK);
  text = memory_sink_text(sink);
  ...
  req.selector_json =
      "{\"and\":[{\"exists\":\"/value\"},"
      "{\"not\":{\"icontains\":{\"field\":\"/value\",\"value\":\"ET\"}}}]}";
  rc = client->query_keys(client, &req, &handler, &capture, &res, &error);
  assert_int_equal(rc, LC_OK);
  assert_int_equal(capture.key_count, 1U);
  assert_string_equal(capture.keys[0], "alpha");
...
static void
test_pouch_endpoint_index_query_excludes_typed_not_eq_candidates(void **state) {
  char root[256];
  char endpoint[320];
  lc_client *client;
  lc_lease *alpha;
  ...
  (void)state;
  test_root_path(root, sizeof(root), "query-typed-not-eq-exclusions");
  test_cleanup_root(root);
  test_endpoint(endpoint, sizeof(endpoint), root);
  memset(&error, 0, sizeof(error));
  memset(&res, 0, sizeof(res));
  ...
  alpha = pouch_acquire_query_key(client, "alpha", &error);
  pouch_save_query_json(alpha, "{\"n\":1,\"flag\":true,\"maybe\":null}",
                        &error);
  bravo = pouch_acquire_query_key(client, "bravo", &error);
  pouch_save_query_json(bravo, "{\"n\":2,\"flag\":false,\"maybe\":\"set\"}",
                        &error);
  charlie = pouch_acquire_query_key(client, "charlie", &error);
  pouch_save_query_json(charlie, "{\"n\":3}", &error);
  alpha->close(alpha);
  bravo->close(bravo);
  charlie->close(charlie);

  lc_query_req_init(&req);
  req.selector_json = "{\"and\":[{\"exists\":\"/flag\"},"
                      "{\"not\":{\"eq\":{\"field\":\"/flag\","
                      "\"value\":true}}}]}";
  rc = client->query_keys(client, &req, &handler, &capture, &res, &error);
  assert_int_equal(rc, LC_OK);
  ...
  memset(&capture, 0, sizeof(capture));
  lc_query_req_init(&req);
  req.selector_json = "{\"and\":[{\"exists\":\"/maybe\"},"
                      "{\"not\":{\"eq\":{\"field\":\"/maybe\","
                      "\"value\":null}}}]}";
  rc = client->query_keys(client, &req, &handler, &capture, &res, &error);
  assert_int_equal(rc, LC_OK);
...
static void test_pouch_endpoint_index_query_filters_key_selector(void **state) {
  char root[256];
  char endpoint[320];
  lc_client *client;
  lc_lease *alpha;
  ...
  (void)state;
  test_root_path(root, sizeof(root), "query-index-key");
  test_cleanup_root(root);
  test_endpoint(endpoint, sizeof(endpoint), root);
  memset(&error, 0, sizeof(error));
  memset(&res, 0, sizeof(res));
...
static void test_pouch_endpoint_index_exists_query_keys_pages(void **state) {
  char root[256];
  char endpoint[320];
  lc_client *client;
  lc_client *other_client;
  ...
  (void)state;
  test_root_path(root, sizeof(root), "query-index-exists-keys-pages");
  test_cleanup_root(root);
  test_endpoint(endpoint, sizeof(endpoint), root);
  memset(&error, 0, sizeof(error));
  memset(&res, 0, sizeof(res));
  ...
  alpha = pouch_acquire_query_key(client, "alpha", &error);
  pouch_save_query_json(alpha, "{\"indexed\":true}", &error);
  delta = pouch_acquire_query_key(client, "delta", &error);
  pouch_save_query_json(delta, "{\"other\":true}", &error);
  bravo = pouch_acquire_query_key(client, "bravo", &error);
  pouch_save_query_json(bravo, "{\"indexed\":false}", &error);

  handler.begin = query_key_capture_begin;
  handler.chunk = query_key_capture_chunk;
  handler.end = query_key_capture_end;
  lc_query_req_init(&req);
  req.selector_json = "{\"exists\":\"/indexed\"}";
  req.limit = 2L;
  rc = client->query_keys(client, &req, &handler, &capture, &res, &error);
  assert_int_equal(rc, LC_OK);
  assert_int_equal(capture.key_count, 2U);
...
static void test_pouch_endpoint_index_in_query_keys_pages(void **state) {
  ...
  other_client = open_pouch_client_with_namespace(endpoint, "aard");
  other_doc = pouch_acquire_query_key(other_client, "aardvark", &error);
  pouch_save_query_json(other_doc, "{\"region\":\"north\"}", &error);
  other_doc->close(other_doc);
  other_client->close(other_client);
...
static void test_pouch_endpoint_index_prefix_query_keys_pages(void **state) {
  char root[256];
  char endpoint[320];
  lc_client *client;
  lc_client *other_client;
  ...
  (void)state;
  test_root_path(root, sizeof(root), "query-index-prefix-keys-pages");
  test_cleanup_root(root);
  test_endpoint(endpoint, sizeof(endpoint), root);
  memset(&error, 0, sizeof(error));
  memset(&res, 0, sizeof(res));
  ...
  alpha = pouch_acquire_query_key(client, "alpha", &error);
  pouch_save_query_json(alpha, "{\"name\":\"alpha\"}", &error);
  delta = pouch_acquire_query_key(client, "delta", &error);
  pouch_save_query_json(delta, "{\"name\":\"beta\"}", &error);
  bravo = pouch_acquire_query_key(client, "bravo", &error);
  pouch_save_query_json(bravo, "{\"name\":\"alto\"}", &error);

  handler.begin = query_key_capture_begin;
  handler.chunk = query_key_capture_chunk;
  handler.end = query_key_capture_end;
  lc_query_req_init(&req);
  req.selector_json = "{\"prefix\":{\"field\":\"/name\",\"value\":\"al\"}}";
  req.limit = 2L;
  rc = client->query_keys(client, &req, &handler, &capture, &res, &error);
  assert_int_equal(rc, LC_OK);
  assert_int_equal(capture.key_count, 2U);
...
static void test_pouch_endpoint_index_contains_query_keys_pages(void **state) {
  char root[256];
  char endpoint[320];
  lc_client *client;
  lc_client *other_client;
  ...
  (void)state;
  test_root_path(root, sizeof(root), "query-index-contains-keys-pages");
  test_cleanup_root(root);
  test_endpoint(endpoint, sizeof(endpoint), root);
  memset(&error, 0, sizeof(error));
  memset(&res, 0, sizeof(res));
  ...
  alpha = pouch_acquire_query_key(client, "alpha", &error);
  pouch_save_query_json(alpha, "{\"body\":\"benchmark\"}", &error);
  delta = pouch_acquire_query_key(client, "delta", &error);
  pouch_save_query_json(delta, "{\"body\":\"baseline\"}", &error);
  bravo = pouch_acquire_query_key(client, "bravo", &error);
  pouch_save_query_json(bravo, "{\"body\":\"marked\"}", &error);

  handler.begin = query_key_capture_begin;
  handler.chunk = query_key_capture_chunk;
  handler.end = query_key_capture_end;
  lc_query_req_init(&req);
  req.selector_json = "{\"contains\":{\"field\":\"/body\",\"value\":\"mark\"}}";
  req.limit = 2L;
  rc = client->query_keys(client, &req, &handler, &capture, &res, &error);
  assert_int_equal(rc, LC_OK);
  assert_int_equal(capture.key_count, 2U);
...
static void
test_pouch_endpoint_index_text_query_streams_documents_pages(void **state) {
  char root[256];
  char endpoint[320];
  lc_client *client;
  lc_client *other_client;
  ...
  (void)state;
  test_root_path(root, sizeof(root), "query-index-text-docs-pages");
  test_cleanup_root(root);
  test_endpoint(endpoint, sizeof(endpoint), root);
  memset(&error, 0, sizeof(error));
  memset(&res, 0, sizeof(res));
  ...
  pouch_save_query_json(charlie, "{\"name\":\"alpine\",\"body\":\"landmark\"}",
                        &error);
  alpha = pouch_acquire_query_key(client, "alpha", &error);
  pouch_save_query_json(alpha, "{\"name\":\"alpha\",\"body\":\"benchmark\"}",
                        &error);
  delta = pouch_acquire_query_key(client, "delta", &error);
  pouch_save_query_json(delta, "{\"name\":\"beta\",\"body\":\"baseline\"}",
                        &error);
  bravo = pouch_acquire_query_key(client, "bravo", &error);
  pouch_save_query_json(bravo, "{\"name\":\"alto\",\"body\":\"marked\"}",
                        &error);
...
test_pouch_endpoint_index_rebuild_writes_temporal_generation(void **state) {
  ...
  test_query_temporal_generation_path(root, "other", generation_path,
                                      sizeof(generation_path));
  test_read_temporal_generation_file(generation_path, &generation);
...
test_pouch_endpoint_index_rebuild_writes_exact_generation(void **state) {
  ...
  (void)state;
  test_root_path(root, sizeof(root), "query-index-exact-generation");
  test_cleanup_root(root);
  test_endpoint(endpoint, sizeof(endpoint), root);
  memset(&error, 0, sizeof(error));
  memset(&res, 0, sizeof(res));
  ...
  client = open_pouch_client(endpoint);
  alpha = pouch_acquire_query_key(client, "alpha", &error);
  pouch_save_query_json(alpha, "{\"value\":\"hot\"}", &error);
  bravo = pouch_acquire_query_key(client, "bravo", &error);
  pouch_save_query_json(bravo, "{\"value\":\"hot\"}", &error);
  charlie = pouch_acquire_query_key(client, "charlie", &error);
  ...
  other_client = open_pouch_client_with_namespace(endpoint, "other");
  other_doc = pouch_acquire_query_key(other_client, "aardvark", &error);
  pouch_save_query_json(other_doc, "{\"value\":\"hot\"}", &error);
  other_doc->close(other_doc);
  other_client->close(other_client);
  ...
  handler.end = query_key_capture_end;
  lc_query_req_init(&req);
  req.selector_json = "{\"eq\":{\"field\":\"/value\",\"value\":\"hot\"}}";
  rc = client->query_keys(client, &req, &handler, &capture, &res, &error);
  assert_int_equal(rc, LC_OK);
  assert_int_equal(capture.key_count, 2U);
  assert_string_equal(capture.keys[0], "alpha");
  ...
  test_query_exact_generation_path(root, "default", generation_path,
                                   sizeof(generation_path));
  test_read_exact_generation_file(generation_path, &generation);
  assert_string_equal(generation.namespace_name, "default");
  assert_true(generation.identity.sequence > 0U);
  assert_true(lc_pouch_index_term_table_find(
      &generation.terms, "7:default:6:/value", "s:hot", &term_id));
  ...
  test_query_exact_generation_path(root, "other", generation_path,
                                   sizeof(generation_path));
  test_read_exact_generation_file(generation_path, &generation);
  assert_string_equal(generation.namespace_name, "other");
  assert_true(lc_pouch_index_term_table_find(
      &generation.terms, "5:other:6:/value", "s:hot", &term_id));
  assert_true(lc_pouch_index_term_posting_table_decode(
      NULL, &generation.postings, term_id, &doc_ids));
  assert_int_equal(doc_ids.count, 1U);
  assert_int_equal(doc_ids.items[0], 0U);
  ...
  client = open_pouch_client(endpoint);
  delta = pouch_acquire_query_key(client, "delta", &error);
  pouch_save_query_json(delta, "{\"value\":\"hot\"}", &error);
  delta->close(delta);
  memset(&capture, 0, sizeof(capture));
  memset(&res, 0, sizeof(res));
  rc = client->query_keys(client, &req, &handler, &capture, &res, &error);
...
test_pouch_endpoint_index_rebuild_writes_exists_generation(void **state) {
  ...
  other_client = open_pouch_client_with_namespace(endpoint, "other");
  other_doc = pouch_acquire_query_key(other_client, "aardvark", &error);
  pouch_save_query_json(other_doc, "{\"flag\":true}", &error);
  other_doc->close(other_doc);
  other_client->close(other_client);
  ...
  test_query_exists_generation_path(root, "other", generation_path,
                                    sizeof(generation_path));
  test_read_exact_generation_file(generation_path, &generation);
  assert_string_equal(generation.namespace_name, "other");
  assert_true(lc_pouch_index_term_table_find(&generation.terms,
                                             "5:other:5:/flag", "", &term_id));
  assert_true(lc_pouch_index_term_posting_table_decode(
      NULL, &generation.postings, term_id, &doc_ids));
  assert_int_equal(doc_ids.count, 1U);
  assert_int_equal(doc_ids.items[0], 0U);
...
static void
test_pouch_endpoint_index_rebuild_writes_number_generation(void **state) {
  char root[256];
  char endpoint[320];
  char query_path[512];
  char generation_path[512];
  ...
  (void)state;
  test_root_path(root, sizeof(root), "query-index-number-generation");
  test_cleanup_root(root);
  test_endpoint(endpoint, sizeof(endpoint), root);
  memset(&error, 0, sizeof(error));
  memset(&res, 0, sizeof(res));
  ...
  client = open_pouch_client(endpoint);
  alpha = pouch_acquire_query_key(client, "alpha", &error);
  pouch_save_query_json(alpha, "{\"amount\":1}", &error);
  bravo = pouch_acquire_query_key(client, "bravo", &error);
  pouch_save_query_json(bravo, "{\"amount\":2}", &error);
  charlie = pouch_acquire_query_key(client, "charlie", &error);
  pouch_save_query_json(charlie, "{\"amount\":5}", &error);
  delta = pouch_acquire_query_key(client, "delta", &error);
  pouch_save_query_json(delta, "{\"amount\":11}", &error);
  echo = pouch_acquire_query_key(client, "echo", &error);
  pouch_save_query_json(echo, "{\"value\":\"missing-amount\"}", &error);
  alpha->close(alpha);
  bravo->close(bravo);
  charlie->close(charlie);
  delta->close(delta);
  ...
  handler.end = query_key_capture_end;
  lc_query_req_init(&req);
  req.selector_json = "{\"range\":{\"field\":\"/amount\",\"gte\":2,\"lt\":10}}";
  rc = client->query_keys(client, &req, &handler, &capture, &res, &error);
  assert_int_equal(rc, LC_OK);
  assert_int_equal(capture.key_count, 2U);
  assert_string_equal(capture.keys[0], "bravo");
  ...
  assert_string_equal(generation.namespace_name, "default");
  assert_true(generation.identity.sequence > 0U);
  assert_true(lc_pouch_index_number_posting_table_has_field(
      &generation.postings, "/amount"));
  ...
  test_stale_number_generation_file(generation_path);
  client = open_pouch_client(endpoint);
  memset(&capture, 0, sizeof(capture));
  memset(&res, 0, sizeof(res));
  rc = client->query_keys(client, &req, &handler, &capture, &res, &error);
...
static void
test_pouch_endpoint_index_rebuild_writes_text_generation(void **state) {
  char root[256];
  char endpoint[320];
  char query_path[512];
  char generation_path[512];
  ...
  (void)state;
  test_root_path(root, sizeof(root), "query-index-text-generation");
  test_cleanup_root(root);
  test_endpoint(endpoint, sizeof(endpoint), root);
  memset(&error, 0, sizeof(error));
  memset(&res, 0, sizeof(res));
  ...
  client = open_pouch_client(endpoint);
  alpha = pouch_acquire_query_key(client, "alpha", &error);
  pouch_save_query_json(alpha, "{\"body\":\"alphabet marker\"}", &error);
  bravo = pouch_acquire_query_key(client, "bravo", &error);
  pouch_save_query_json(bravo, "{\"body\":\"alpine\"}", &error);
  charlie = pouch_acquire_query_key(client, "charlie", &error);
  pouch_save_query_json(charlie, "{\"body\":\"landmark\"}", &error);
  delta = pouch_acquire_query_key(client, "delta", &error);
  pouch_save_query_json(delta, "{\"body\":\"other\"}", &error);
  alpha->close(alpha);
  bravo->close(bravo);
  charlie->close(charlie);
  delta->close(delta);
  ...
  other_client = open_pouch_client_with_namespace(endpoint, "other");
  other_doc = pouch_acquire_query_key(other_client, "aardvark", &error);
  pouch_save_query_json(other_doc, "{\"body\":\"algae marker\"}", &error);
  other_doc->close(other_doc);
  other_client->close(other_client);
  ...
  handler.end = query_key_capture_end;
  lc_query_req_init(&req);
  req.selector_json = "{\"prefix\":{\"field\":\"/body\",\"value\":\"al\"}}";
  rc = client->query_keys(client, &req, &handler, &capture, &res, &error);
  assert_int_equal(rc, LC_OK);
  assert_int_equal(capture.key_count, 2U);
  assert_string_equal(capture.keys[0], "alpha");
  ...
  test_query_text_generation_path(root, "default", generation_path,
                                  sizeof(generation_path));
  test_read_text_generation_file(generation_path, &generation);
  assert_string_equal(generation.namespace_name, "default");
  assert_true(generation.identity.sequence > 0U);
  assert_true(lc_pouch_index_text_posting_table_has_field(&generation.postings,
                                                          "/body"));
  assert_true(lc_pouch_index_text_posting_table_append_prefix(
      NULL, &generation.postings, "/body", "al", 0, &doc_ids));
  assert_true(lc_pouch_index_doc_id_set_sort_unique(&doc_ids));
  assert_int_equal(doc_ids.count, 2U);
  lc_pouch_index_doc_id_set_cleanup(NULL, &doc_ids);
  lc_pouch_index_text_generation_cleanup(NULL, &generation);
  ...
  test_query_text_generation_path(root, "other", generation_path,
                                  sizeof(generation_path));
  test_read_text_generation_file(generation_path, &generation);
  assert_string_equal(generation.namespace_name, "other");
  assert_true(lc_pouch_index_text_posting_table_append_prefix(
  ...
  client = open_pouch_client(endpoint);
  echo = pouch_acquire_query_key(client, "echo", &error);
  pouch_save_query_json(echo, "{\"body\":\"almanac marker\"}", &error);
  echo->close(echo);
  memset(&capture, 0, sizeof(capture));
  memset(&res, 0, sizeof(res));
  rc = client->query_keys(client, &req, &handler, &capture, &res, &error);
  ...
  test_corrupt_text_generation_file(generation_path);
  client = open_pouch_client(endpoint);
  req.selector_json = "{\"contains\":{\"field\":\"/body\",\"value\":\"mark\"}}";
  memset(&capture, 0, sizeof(capture));
  memset(&res, 0, sizeof(res));
  ...
  assert_true(lc_pouch_index_text_posting_table_append_contains(
      NULL, &generation.postings, "/body", "mark", 0, &doc_ids));
  assert_true(lc_pouch_index_doc_id_set_sort_unique(&doc_ids));
  assert_int_equal(doc_ids.count, 3U);
  lc_pouch_index_doc_id_set_cleanup(NULL, &doc_ids);
  lc_pouch_index_text_generation_cleanup(NULL, &generation);
  ...
  test_stale_text_generation_file(generation_path);
  client = open_pouch_client(endpoint);
  memset(&capture, 0, sizeof(capture));
  memset(&res, 0, sizeof(res));
  rc = client->query_keys(client, &req, &handler, &capture, &res, &error);
...
test_pouch_endpoint_reports_local_unsupported_surfaces(void **state) {
  ...
  assert_int_equal(rc, LC_OK);
  lc_query_req_init(&query_req);
  query_req.selector_json = "{\"value\":\"alpha\"}";
  rc = client->query(client, &query_req, sink, &query_res, &error);
  assert_pouch_unsupported(rc, &error,
...
test_pouch_endpoint_txn_recovery_preserves_valid_prepare(void **state) {
  ...
  (void)state;
  test_root_path(root, sizeof(root), "txn-recovery-valid-prepare");
  test_cleanup_root(root);
  test_endpoint(endpoint, sizeof(endpoint), root);
  memset(&error, 0, sizeof(error));
  memset(&decision_res, 0, sizeof(decision_res));
  ...
  lc_acquire_req_init(&acquire);
  acquire.key = "txn/valid-prepare-key";
  acquire.owner = "seed";
  acquire.ttl_seconds = 60L;
  rc = client->acquire(client, &acquire, &lease, &error);
  assert_int_equal(rc, LC_OK);
  ...
  acquire.owner = "txn-owner";
  acquire.txn_id = "txn-valid-prepare-1";
  rc = client->acquire(client, &acquire, &lease, &error);
  assert_int_equal(rc, LC_OK);
  seed_pouch_staged_state(client, lease, "{\"value\":2}", &error);
  ...
  memset(&participant, 0, sizeof(participant));
  participant.namespace_name = "default";
  participant.key = "txn/valid-prepare-key";
  lc_txn_decision_req_init(&decision_req);
  ...
  client = open_pouch_client(endpoint);
  assert_pouch_client_state_text(client, "txn/valid-prepare-key",
                                 "{\"value\":1}", &error);
  lc_txn_replay_req_init(&replay_req);
  replay_req.txn_id = "txn-valid-prepare-1";
  rc = client->txn_replay(client, &replay_req, &replay_res, &error);
  assert_int_equal(rc, LC_OK);
  assert_string_equal(replay_res.txn_id, "txn-valid-prepare-1");
...
int main(void) {
  const struct CMUnitTest tests[] = {
      ...
      cmocka_unit_test(test_pouch_endpoint_index_prefix_query_keys_pages),
      ...
      cmocka_unit_test(
          test_pouch_endpoint_index_rebuild_writes_number_generation),
      cmocka_unit_test(
          test_pouch_endpoint_index_rebuild_writes_text_generation),
      cmocka_unit_test(
