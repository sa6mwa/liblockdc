...
| --- | --- | --- | --- | --- |
| `make test-host` as the ordinary local confidence gate | `make finalize-slice` and `make test-all` | Formatting and the narrow debug test gate are the pre-commit slice gate. `test-all` is the parallel functional confidence tier: debug, host, QEMU cross, Valgrind, and deterministic e2e. Fuzzing remains an explicit or prerelease hardening gate because a cold AFL++ bootstrap is not an everyday test invariant. Performance work is explicit through `bench-gate` and intentional prerelease gates. | `lifecycle_command_surface_test` asserts the target, help entry, and graph. | Active |
