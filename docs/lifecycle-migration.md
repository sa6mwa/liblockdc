...
| Manually choosing local deterministic confidence commands | `make prerelease` | The local prerelease proof runs without the initial clean: slice gate, Valgrind, fuzz smoke, lockd e2e, Lua tests, and benchmark gate. Full release-matrix work remains behind `make prerelease-hardening`, `make release-matrix`, and the clean `make release` gate. | `lifecycle_command_surface_test` asserts the deterministic prerelease graph and release-pipeline split. | Active |
...
| Expensive release rehearsal split across long fuzzing, parity, and matrix commands | `make prerelease-hardening` | Full fuzzing, the Pouch Go parity gate, and the release matrix remain available after the deterministic prerelease proof. | `lifecycle_command_surface_test` asserts the hardening graph. | Active |
