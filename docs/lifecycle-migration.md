...
| --- | --- | --- | --- | --- |
| `make test-host` as the ordinary local confidence gate | `make finalize-slice` and `make test-all` | Formatting and the narrow debug test gate are the pre-commit slice gate. `test-all` is the complete functional confidence tier: debug, host, QEMU cross, Valgrind, fuzz smoke, and deterministic e2e. Performance work is explicit through `bench-gate` and intentional prerelease gates, never an implicit functional-test workload. | `lifecycle_command_surface_test` asserts the target, help entry, and graph. | Active |
