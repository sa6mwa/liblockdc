...
`iprefix` or `contains` / `icontains` `or` branches, and mixed numeric `range`
plus `prefix` / `iprefix` or `contains` / `icontains` `or` branches, union
presence, membership, ordered numeric, or text postings before stable
