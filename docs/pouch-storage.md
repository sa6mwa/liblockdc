...
`contains` / `icontains` `or` branches, mixed string `in` plus `prefix` /
`iprefix` `or` branches, and mixed numeric `range` plus `prefix` / `iprefix`
or `contains` / `icontains` `or` branches, union presence, membership, ordered
numeric, or text postings before stable key/cursor ordering is restored.
Mixed
