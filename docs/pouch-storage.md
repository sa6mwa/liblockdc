...
- Current redesigned-pouch implementation status: `query_keys` and document
  ...
  scalar `in`, simple case-sensitive `prefix` and `contains`, simple bounded
  numeric `range`, including `/tags[]` array membership, and field `exists`
  postings before final `liblql` acceptance. Broader typed/text postings and
  richer indexed selector plans remain to be implemented.
