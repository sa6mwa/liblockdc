...
detect_host_release_preset() {
    ...
    if ! triple="$($cc_bin -dumpmachine 2>/dev/null)"; then
        printf 'test_release_source.sh: failed to resolve native compiler triple with %s -dumpmachine\n' "$cc_bin" >&2
        exit 1
    fi
    ...
    case "$triple" in
        ...
        arm*-linux-gnueabihf*|armv7*-linux-gnueabihf*|arm*-linux-gnu*|armv7*-linux-gnu*)
            ...
            ;;
        *)
            printf 'test_release_source.sh: unsupported native compiler triple: %s\n' "$triple" >&2
            exit 1
            ;;
    esac
}
...
external_root="$repo_root/.cache/deps/$host_target_id"
dependency_build_root="$repo_root/.cache/deps-build/$host_target_id"
if [[ ! -d "$external_root" ]]; then
    printf 'test_release_source.sh: dependency root is missing: %s\n' "$external_root" >&2
